...
- Brand cards: use the shared `<x-brand-logo>` component and the immutable files in `public/brand-logos`. Every card uses the same height, borderless ivory field, logo stage, and metadata baseline. The component optically fits the visible SVG artwork to compensate for differing internal viewBox whitespace; never edit a source logo merely to resize it. Brand cards do not use visible sequence numbers or boxed grid lines. Use a verified SVG when available, otherwise retain the local SVG fallback.
