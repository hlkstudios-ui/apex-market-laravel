...
- Filter bars: use `.ui-filter-bar` and apply `.filter-submit.ui-button` only to the direct submit action. Never use a broad descendant selector such as `.filters button`; dropdown triggers and options are buttons too, so that selector destroys their component states. All fields in a row share `--control-height` and align at their top and bottom edges.
