...
<section class="brand-directory">
    ...
    <div class="directory-layout">
        ...
        <div class="directory-content">
            ...
            <div class="brand-groups">@forelse($brands as $letter=>$group)<section id="letter-{{$letter}}"><h2>{{$letter}}</h2><div class="brand-logo-grid">@foreach($group as $brand)<a class="brand-logo-card" href="{{route('store.index',['search'=>$brand->name])}}" aria-label="Discover {{$brand->name}} — {{$brand->niche}}"><x-brand-logo :brand="$brand" /><div class="brand-meta"><small class="brand-niche">{{$brand->niche}}</small><span>Discover <i>↗</i></span></div></a>@endforeach</div></section>@empty<div class="empty"><h2>No maisons found</h2><p>Try another letter or department.</p></div>@endforelse</div>
        </div>
    </div>
</section>
@endsection
