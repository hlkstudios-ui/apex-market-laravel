...
<section class="brand-directory">
    ...
    <div class="directory-layout">
        ...
        <div class="directory-content">
            ...
            <div class="brand-groups">@forelse($brands as $letter=>$group)<section id="letter-{{$letter}}"><h2>{{$letter}}</h2><div class="brand-logo-grid">@foreach($group as $brand)<a class="brand-logo-card" href="{{route('store.index',['search'=>$brand->name])}}"><div class="brand-mark"><img src="{{$brand->logo_url}}" alt="{{$brand->name}} wordmark" loading="lazy" onerror="this.onerror=null;this.src='{{route('brands.mark',$brand)}}'"></div><div><strong>{{$brand->name}}</strong><small>{{$brand->niche}}</small></div><span class="brand-arrow">↗</span></a>@endforeach</div></section>@empty<div class="empty"><h2>No maisons found</h2><p>Try another letter or department.</p></div>@endforelse</div>
        </div>
    </div>
</section>
@endsection
